// daterangepicker.js
// version : 0.0.5
// author : Chunlong Liu / www.jszen.com
// license : MIT
// LastEdit : 2014-09-18 11:11 By Gaze
...
(function($)
{
	...
	$.fn.dateRangePicker = function(opt)
	{
		...
		function init_datepicker()
		{
			...
			{
				$(opt.container).append(box);
				box.addClass('date-picker-inline');
			}
		...
		function checkSelectionValid()
		{
			...
			if (opt.singleDate) { // Validate if only start is there
				if (opt.start && !opt.end)
					box.find('.dpr-selected').removeClass('error').addClass('normal');
				else
					box.find('.dpr-selected').removeClass('error').removeClass('normal');
			}
			else if ( opt.maxDays && days > opt.maxDays)
			{
				...
				box.find('.dpr-selected').removeClass('normal').addClass('error').find('.error-top').html( lang('less-than').replace('%d',opt.maxDays) );
			}
			else if ( opt.minDays && days < opt.minDays)
			{
				...
				opt.end = false;
				box.find('.day').removeClass('checked');
				box.find('.dpr-selected').removeClass('normal').addClass('error').find('.error-top').html( lang('more-than').replace('%d',opt.minDays) );
			}
			else
			{
				if (opt.start || opt.end)
		...
		function showMonth(date,month)
		{
			...
			var yearsName = nameYears();
			// box.find('.'+month+' .month-name').html(monthName+' '+date.getFullYear());
			box.find('.'+month+' .month-name').html(date.getFullYear()+yearsName+monthName);
		...
		{
			var timeHtml = '<div class="time-selected">'
				+'<em class="ico-time-sm">Time</em><span class="hour-val">00</span><i>:</i><span class="minute-val">00</span>'
				+'</div>'
				+'<div class="hour">'
				+'<label>Hour</label><input type="range" class="hour-range" name="hour" min="0" max="23">'
				+'</div>'
				+'<div class="minute">'
				+'<label>Minute</label><input type="range" class="minute-range" name="minute" min="0" max="59">'
				+'</div>';
			return timeHtml;
		}
		...
		function createDom()
		{
			...
			{
				html += '<div class="dpr-shortcuts"><b>'+lang('shortcuts')+'</b>';
				...
				if (data)
				{
					...
					{
						html += '<span class="prev-days"><i>'+lang('past')+'</i>';
					...
					{
						html += '<span class="next-days"><i>'+lang('following')+'</i>';
			...
			html +='<div class="month-wrapper">'
				+'<table class="month1"><thead><tr class="caption"><th style="width:27px;"><span class="prev">&lt;</span></th><th colspan="5" class="month-name">January, 2011</th><th style="width:27px;"><span class="next">&gt;</span></th></tr><tr class="week-name">'+getWeekHead()+'</thead><tbody></tbody></table>'
			if ( ! opt.singleDate ) {
				html += '<div class="gap">'+getGapHTML()+'</div>'
				+'<table class="month2"><thead><tr class="caption"><th style="width:27px;"><span class="prev">&lt;</span></th><th colspan="5" class="month-name">January, 2011</th><th style="width:27px;"><span class="next">&gt;</span></th></tr><tr class="week-name">'+getWeekHead()+'</thead><tbody></tbody></table>'
			}
			if (opt.time.enabled){
				html +='<div class="time">'
					+'<div class="time1"></div>'
