            ...
            <div class="row">
                <div class="col-md-12" role="main" id="main">
                    ...
                    <form id="saveForm" class="form-horizontal parsley-form"
                          method="post" name="saveForm" action="authorization.do">
                        <input type="hidden" name="ids" value="">
                        <div class="row area-list-dec">
                            <div class="area-search">

                                <div class="form-select">
                                    <select id="roleid1" name="id" class="form-control"  placeholder="角色" >
                                        <c:forEach items="${roles}"   var="role">
                                            <option statu="${role.status}" selected='selected'    value="${role.id}">${role.name}</option>
                                        </c:forEach>
                                    </select>
                                </div>
                                ...
                                <c:forEach  items="${functions}"  var="item">
                                    <tr>
                                        <c:if test="${item.parentId== 0}">
                                            <td style="width: 100px;"><label title="${item.name}" class="icheck-inline">
                                            ...
                                            <td>
                                                <c:forEach items="${functions}"  var="child">
                                                    <c:if test="${child.parentId==item.id}">
...
//	    return(localhostPath+projectName);
    ...
    $(function(){
        ...
        $('#roleid1').change(function(){
            ...
            //查询当前角色对应的功能
            $.ajax({url:"${pageContext.request.contextPath}/backer/system!findFactFunByRole.action",data:{roleid:+$("#roleid1").val()},type:"post",success:function(data){
...
<div id="save_edit" class="modal fade">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form id="saveEditForm" class="form-horizontal parsley-form" method="post"  action="edit.do"  data-validate="parsley">
...
<div id="save_update" class="modal fade">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form id="saveupdateForm" class="form-horizontal parsley-form" method="post"  action="edit.do"  data-validate="parsley">
                <input type="hidden" name="id"  id="roleid"/>
                <div class="modal-body">
                    <div class="form-group">
                        <label class="col-md-3 control-label"><i class="required">*</i>角色名称</label>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-md-7">
                            <label title="" class="icheck-inline">
                                <input type="radio" id="sta" name="status" value="1" class="icheck-input fundidchild" >启用
                                <input type="radio" id="sta" name="status" value="0" class="icheck-input fundidchild" >禁用
                            </label>
                        </div>
                    </div>
                </div>
    ...
    <div class="modal-dialog w550">
        <div class="modal-content">
            ...
            <form id="authRolesForm" class="form-horizontal" role="form"
                  action="${pageContext.request.contextPath}/backer/system!editRoleFunction" method="post">
                        ...
                        <div class="col-md-12 ui-icheck-label">
                            <c:forEach items="${functions}" var="item">
                                <label title="${item.name}" class="icheck-inline">
                                    <input type="checkbox" name="ids" class="icheck-input" value="${item.id}">${item.name}
                                </label>
                            </c:forEach>
                        </div>
                    </div>
