...
public class PageDeal {
	...
	public String getPageStr() {
		...
		if (totalResult > 0) {
			sb.append(" <ul>\n");
			if (currentPage == 1) {
				sb.append(" <td class=\"center\">首页&nbsp;&nbsp;&nbsp;&nbsp;");
				sb.append(" <a href=\"#\">上页</a>");
			} else {
			...
			sb.append("				var reg = /page.currentPage=\\d*/g;\n");
			sb.append("				url = url.replace(reg,'page.currentPage=');\n");
			sb.append("			}else{\n");
			sb.append("				url += \"&"
					+ (entityOrField ? "page.currentPage"
							: "page.currentPage") + "=\";\n");
			sb.append("			}\n");
			sb.append("		}else{url += \"?"
					+ (entityOrField ? "page.currentPage"
